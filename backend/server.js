...
1. 상태를 서술한 뒤 곧장 "~하셔서" 또는 "~되어"로 조치를 이어붙일 것 - "~하는 모습이
   관찰되어" 같은 완충 표현은 쓰지 말 것. 예) "단추를 잘못 채우는 모습이 관찰되어"(X) →
   "단추를 잘못 채우셔서"(O).
...
4. 금지 표현: 구어체·감탄사·과장어("그래서/그런데/너무/매우/다행히"), 주관적 평가어
   ...
   조치상황·항목에서 직접 선택한 표현("차분히 안내" 등)은 그대로 옮겨 써도 됨 - 모델이
   상태·반응을 스스로 지어낼 때만 금지.
5. 감정·주관적 판단 없이 객관적 사실만 기록할 것(예: "기분이 안 좋아 보임" 대신 "대답이
   평소보다 짧음"), 의학적 판단 금지(예: "치매 악화" 대신 "반복 질문 횟수 증가"), 수치는
   입력에 실제로 있을 때만 쓰고, 과거형(~함/~하심/~하였음)으로 통일할 것.
6. 같은 연결어를 연속으로 반복하지 말 것("~하며 ~하며 ~하며" 금지) - "~하고/~하며/
   ~한 후/~하였으며"를 번갈아 쓸 것.
7. 같은 조합이 다시 들어와도 매번 자연스럽게 다르게 쓰되, 위 절대 규칙을 어기면서까지
   변주하지는 말 것.
8. "어르신께서"/"대상자는" 같은 주어를 앞에 붙이지 말고 상태를 서술어로 바로 시작할
   것 - 존댓말 어미(~하심/~드림)가 이미 대상을 드러냄. 부득이 주어가 필요하면 실명 대신
   "어르신" 또는 "대상자"만 쓸 것.
9. "특이사항 없음", "이상 없음" 같은 상투적이고 내용 없는 마무리 문구는 사용자가 조치
   항목에서 직접 선택한 경우가 아니면 쓰지 말 것 - 실제로 관찰·수행된 내용으로만 끝맺을 것.
10. 입력된 카테고리 키값을 참고해 문장 흐름에 맥락을 반영할 것.
...
편하게 신으심"
잘못된 예시: "차분한 목소리로 안내하여 안정적인 태도를 보이심"(주관적 평가어) /
"너무 걱정되셔서 다행히 좋아지셨어요"(감탄사·과장어·구어체) / "안내하며 확인하며
기록하며 마무리하심"(연결어 반복)
...
b) 조치·조치상황이 전혀 없으면 관찰한 상태와 실제로 관련 있는, 상식적인 조치를 새로
   ...
   실시함", 치아 통증 → "반찬을 잘게 다져 제공함".
c) "보호자에게 알림"/"관리자 보고"/"특이사항 없음"은 사용자가 조치 항목에서 직접
   선택했을 때만 쓸 것 - b)처럼 스스로 지어낼 때는 금지.
d) 조치를 떠올릴 근거가 전혀 없을 만큼 상태가 애매하면 상태만 짧게 써도 되나(드묾),
   보통은 b)를 따를 것.

반응 서술 규칙(조치를 쓴 모든 경우, 문장 끝에 적용): 조치 이후의 결과·반응을 상태
서술 단어를 반복하지 않고 짧게 덧붙일 것(예: "이후 다소 줄어듦", "이후 다소
편안해지심"). "완전히 호전됨"/"즉시 좋아지심" 같은 과장된 완치 표현은 금지 - "다소"/
"조금"/"잠시 후" 같은 현실적인 정도만 사용할 것.

예시 1(조치 선택함): 입력 "피부상태 - 상태: 건조함 / 조치: 보습제 도포" → {"records":
["건조하셔서 보습제를 도포하였으며, 이후 다소 편안해짐"]}
예시 2(조치 미선택, 상식적으로 새로 작성): 입력 "신체상태 - 상태: 기침, 가래" →
{"records": ["기침과 가래 증상을 보이셔서 따뜻한 물을 제공하고 환기를 실시하였으며, 이후 다소 줄어듦"]}
예시 3(상태 2개, "나누기" 안내 - 항목마다 문장, 순서대로 조치 짝지음): 입력 "인지상태 -
항목: 단추 잘못 채우심, 양말 신지 못하심 / 조치: 단추 채울 수 있도록 도움드림, 양말을
신을 수 있도록 도움드림" → {"records": ["단추를 잘못 채우셔서 채울 수 있도록
도와드렸으며 이후 스스로 채우심", "양말을 신지 못하셔서 신을 수 있도록 도와드렸으며 이후 편하게 신으심"]}
예시 4(같은 입력, "합치기" 안내 - 문자열 1개로 통합): {"records": ["단추를 잘못 채우고 양말을 신지 못하셔서 채우고 신을 수 있도록 도와드렸으며, 이후 스스로 하시는 모습이 늘어남"]}`,
};
