...
// [18] 지침 구조 개편: 기록유형(record_type)마다 지침을 통째로 새로 쓰는
// 대신, record_type과 무관하게 항상 적용되는 공통지침(COMMON_GUIDELINE)과
// 번호로 관리하는 재사용 가능한 지침 조각(GUIDELINES)으로 나눈다.
// 기록유형별로는 RECORD_TYPE_GUIDELINE_IDS에서 "어떤 번호를 쓸지"만
// 지정하므로, 여러 기록유형이 같은 지침 조각을 공유할 수도 있고 특정
// 유형 전용 조각만 쓸 수도 있다. 지금은 GUIDELINES[1](조치 작성 규칙)
// 하나뿐이라 모든 기록유형이 동일하게 동작하지만(기존 동작과 100% 동일),
// 기록유형 전용 지침이 생기면 새 번호로 추가하고 매핑만 바꾸면 된다.
...
status 절대 규칙: 입력된 관찰 항목에 없는 증상·부위·수치·행동·감정 변화는 status에
...
7. "이에 따라 / 이후 / 확인 결과" 등 자연스러운 연결어를 각 필드 안에서 사용할 것(action이
   빈 문자열인 경우는 해당 없음).
8. 선택 항목 수에 따라 문장 길이를 조절할 것 - 각 필드 1~2문장으로 간결하게 쓰고, 어떤 경우에도
   필드당 최대 2문장을 넘지 말 것. 단순 나열은 절대 금지.
9. 선택 항목을 단순 나열하지 말고 하나의 상황으로 통합하여 서술할 것(단, 항목이 1개뿐이면
...
const GUIDELINES = {
  // 조치(action) 필드 작성 규칙. 선택된 조치가 있으면 그대로 반영하고,
  // 없으면 상태에 맞는 상식적인 조치를 자유롭게 새로 쓰되 "보호자에게
  // 알림"/"관리자 보고"/"특이사항 없음" 같은 상투적 표현은 직접 선택했을
  // 때만 허용한다(이번 세션에 여러 차례 사용자와 조율해 확정한 규칙).
  1: `"action" 필드 작성 규칙(중요, 순서대로 확인):
a) 입력에 조치(action)·조치상황(care_level) 항목이 있으면 - 그 내용을 바탕으로 실제
   수행한 것처럼 서술할 것.
b) 조치·조치상황 항목이 전혀 없으면 - status에 서술한 상태를 보고, 요양보호사가 그
   어르신을 위해 상식적으로 해줄 수 있는 구체적인 조치를 자연스럽게 새로 작성할 것.
   그 상태와 실제로 관련 있는 조치여야 하며 아무거나 지어내면 안 됨. 예)
   기침·가래 → "따뜻한 물을 제공하고 환기를 실시함", 치아 통증 → "반찬을 잘게 다져
   제공함", 실내에만 계심 → "실버체조 스트레칭을 도와드림".
...
d) status도 관련 조치를 떠올릴 근거가 전혀 없을 만큼 애매하면 action을 빈 문자열("")로
   반환해도 되지만, 이런 경우는 드뭅니다 - status가 있으면 보통 b)를 따르세요.

결과 작성 규칙(action이 빈 문자열이 아닌 모든 경우에 적용 - a)~b) 중 어느 경우로 action을
작성했든 마지막에 반드시 적용):
- 조치 내용 뒤에, 그 조치를 취한 이후의 결과·반응을 문장 끝에 반드시 덧붙일 것. 결과는
  앞서 status에 적은 상태와 실제로 관련 있는 내용이어야 함. 예)
  기침·가래 상태 → "이후 기침이 다소 줄어든 모습이 관찰됨", 식욕 없음 상태 → "이후 식사를
  잘 하심", 통증 호소 상태 → "이후 조금 편안해지신 모습이 관찰됨".
...
입력: "- 피부상태:\\n  상태(status): 건조함\\n  조치(action): 보습제 도포"
출력: {"status": "어르신의 피부 상태를 확인한 결과 건조함이 관찰됨", "action": "보습제를 도포하였으며, 이후 피부가 다소 편안해진 모습이 관찰됨"}

예시 2(조치를 선택하지 않았을 때 - 상태에 맞는 상식적인 조치를 새로 작성 + 결과를 끝에 덧붙임):
입력: "- 신체상태:\\n  상태(status): 기침, 가래"
출력: {"status": "어르신께서 기침과 가래 증상을 보이심", "action": "따뜻한 물을 제공하고 환기를 실시하였으며, 이후 기침이 다소 줄어든 모습이 관찰됨"}`,
};
