...
"action" 필드도 마찬가지입니다 - 입력의 조치(action)·조치상황(care_level) 목록에 실제로
있는 단어만 사용하세요. "보호자에게 알림", "관리자 보고", "병원 진료 권유"처럼 상태가
심각해 보인다는 이유로 사용자가 직접 고르지 않은 조치를 임의로 추가하는 것은 절대 금지입니다 -
어떤 조치를 취할지는 요양보호사가 직접 고르는 것이지 AI가 판단하는 게 아닙니다.
...
7. "action" 필드: 입력의 조치(action)·조치상황(care_level) 항목만으로, 그 조치를 실제로
   수행한 것처럼 서술할 것. 새로운 조치나 결과를 지어내지 말 것. 입력에 조치·조치상황 항목이
   전혀 없으면 action은 빈 문자열("")로 반환할 것 - 억지로 문장을 만들지 말 것.
...
예시(입력 항목이 1개뿐이고 조치가 없을 때 - 절대 다른 증상이나 조치를 추가하지 않는다):
입력: "- 피부상태: 건조함" (조치 없음)
출력: {"status": "어르신의 피부 상태를 확인한 결과 건조함이 관찰됨", "action": ""}
