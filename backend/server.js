...
1. 문장 구조: 상태(관찰) → 조치(안내·지원·확인) → 반응 순서로 이어지는 문장을 작성할
   ...
   채우셔서 ~"(O).
2. 선택된 상태(관찰) 항목이 여러 개일 때 문장을 하나로 합칠지 항목마다 나눌지는 사용자
   메시지의 "문장 작성 방식" 안내를 그대로 따를 것 - 안내된 문장 개수와 정확히 같은
   개수만큼 records 배열에 문자열을 담을 것(합치라고 안내받으면 문자열 1개, 나누라고
   안내받으면 안내된 개수만큼). 각 문장은 40~50자 내외로 간결하게 작성하고, 불필요한
   수식어·상투적인 연결구를 넣어 늘리지 말고 핵심 정보만 남길 것.
...
반드시 아래 JSON 객체 하나만 반환하세요. records 배열 안의 문자열 개수는 사용자 메시지의
"문장 작성 방식" 안내에서 말한 문장 개수와 정확히 같아야 합니다. 그 외 다른 설명·인사말·
목록 형식·마크다운·코드블록은 절대 포함하지 마세요.
{"records": ["문장1", "문장2", "..."]}`;
...
a) 입력에 조치(action)·조치상황(care_level) 항목이 있으면 - 그 내용을 바탕으로 실제
   수행한 것처럼 서술할 것. 상태·조치 항목이 여러 개이고 문장을 나누는 경우(사용자
   메시지의 "문장 작성 방식" 안내가 나누라고 한 경우)면, 상태와 조치를 나열된 순서대로
   하나씩 짝지어 각 문장에 반영할 것(상태 1번째 항목 → 조치 1번째 항목, 상태 2번째 항목
   → 조치 2번째 항목 ...) - 조치 개수가 상태 개수보다 적으면 남는 상태 문장에는 b)처럼
   상식적으로 어울리는 조치를 새로 지어낼 것. 문장을 하나로 합치는 경우(안내가 합치라고
   한 경우)면 조치 항목들을 자연스럽게 한 문장 안에 함께 녹여 서술할 것.
