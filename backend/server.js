...
8. "이에 따라 / 이후 / 확인 결과" 등 자연스러운 연결어를 각 필드 안에서 사용할 것(action이
   빈 문자열인 경우는 해당 없음).
...
12. "특이사항 없음", "이상 없음", "이후 특이사항 없이 서비스를 마침", "지속적으로 관찰하며
    서비스를 제공함" 같은 상투적이고 내용 없는 마무리 문구는 절대 쓰지 말 것(단, 7-c의
    예외인 경우는 제외). 실제로 관찰되거나 수행된 내용으로만 문장을 끝맺을 것.
13. 입력된 카테고리 키값을 참고하여 문장 흐름에 맥락을 반영할 것.
14. 어르신의 이름을 문장에 직접 표기하지 말 것 - 이름 대신 항상 "어르신" 또는 "대상자"로 표현할 것.

예시 1(조치 항목을 선택했을 때 - 그 내용을 그대로 반영):
입력: "- 피부상태:\n  상태(status): 건조함\n  조치(action): 보습제 도포"
출력: {"status": "어르신의 피부 상태를 확인한 결과 건조함이 관찰됨", "action": "보습제를 도포함"}

예시 2(조치를 선택하지 않았을 때 - 상태에 맞는 상식적인 조치를 새로 작성):
입력: "- 신체상태:\n  상태(status): 기침, 가래"
출력: {"status": "어르신께서 기침과 가래 증상을 보이심", "action": "따뜻한 물을 제공하고 환기를 실시함"}
