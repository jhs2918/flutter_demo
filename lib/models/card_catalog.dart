/// [상태변화일지 전용판] assets/data/cards_visit.json·cards_day.json을 읽어
/// 만든 카드 데이터 모델. 시설(방문요양/주간보호) → 기록유형(상태변화일지
/// 하나) → 카테고리 → 그룹 → 항목의 4단 구조이며, 컴포넌트에는 한글을
/// 하드코딩하지 않고 전부 이 데이터를 통해 렌더링한다. 등급 개념은 이
/// 구조에는 없다(등급별 강조·분리 없음).
library;
